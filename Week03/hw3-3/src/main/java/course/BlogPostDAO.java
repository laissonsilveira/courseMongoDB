...
public class BlogPostDAO {
	...
	if (fieldIsEmpty(username, body, permalink)
    ...
    private boolean fieldIsEmpty(String... fields) {
	boolean valid = false;
	...
	if (fieldIsEmpty(name, body, permalink)) {
	    System.out
	    .println("Fields name or body or permalink can't is empty!");
	    return;
	}

	Bson filter = eq("permalink", permalink);

	Document comments = new Document("body", body);
	comments.put("author", name);
	if (!fieldIsEmpty(email)) {
	    comments.put("email", email);
	}
	Document fields = new Document("comments", comments);
	Document update = new Document("$push", fields);
